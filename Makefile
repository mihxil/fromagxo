...
fromagxo.epub: fromepub.tex fromagxotex.tks
	pandoc $< -o $@


.PHONY: clean
clean:
	rm -fr *.dvi *.ps fromtks.fun fromagxotex.tks *.aux node*.html *.log fromagxo_.ps WARNINGS fromagxo.html  *.pl images.tex index.html fromagxox.tks *.gz unu_dosiero *.pdf *.epub
